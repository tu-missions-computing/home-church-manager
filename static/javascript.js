...
$('#confirmModal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this);
    modal.find('.modal-body').text('¿Estás seguro de que desea eliminar ' + firstname + ' ' + lastname + '?');
...
$('#homegroupModal').on('show.bs.modal', function (event) {
    ...
    // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
    var modal = $(this);
    modal.find('.modal-body').text('¿Estás seguro de que desea eliminar ' + homegroup + '?');
    modal.find('#modal-confirm').attr("href", url)
});
});
